  ...
  return (
    <>
      <div className="c-team">
        <h2 className="c-team__title">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Sequi, voluptates ad?</h2>
        <DividerSide />
        <div className="c-team__holder">
          {ourTeam.map((member, index) => {
            return (
              <div className={`c-member__image-wrapper ${hoverState === index ? 'active' : ''}`} key={`${index}img`}>
          ...
          {ourTeam.map((member, index) => {
            return (
              <motion.div className={`c-member__name ${hoverState === index ? 'active' : ''} `}
                initial="initial"
                whileHover="hover"
                animate="initial"
              ...
              >
                ...
                <motion.p className="c-member__desc" variants={heightMotion}>{member.desc}</motion.p>
              </motion.div>
            )
          })}
        </div>
